...
def shan_to_shan_xiang(shan: str, xiang: str = None) -> str:
    ...
    if shan in full_shan_xiang_map:
        return full_shan_xiang_map[shan]
    
    # 默认为地支山向
    if shan in SHAN_TO_SHAN_XIANG:
        return SHAN_TO_SHAN_XIANG[shan]
    
    return f"{shan}山{xiang or '未知'}向" if xiang else f"{shan}山"


def get_shan_xiang_list(use_24_shan: bool = False) -> list:
    """
    获取山向列表
    
    Args:
        use_24_shan: 是否使用二十四山（默认False，使用十二山向）
        
    Returns:
        山向列表
    """
    if use_24_shan:
        return SHAN_XIANG_24
    else:
        return SHAN_XIANG_12
