...
class ZeriApp:
    ...
    def create_simple_date_test_window(self):
        ...
        # 标题
        ttk.Label(main_frame, text="日期测试窗口", font=("微软雅黑", 16, "bold")).pack(pady=10)
        
        # 输入区域
        input_frame = ttk.LabelFrame(main_frame, text="日期输入", padding="20")
        ...
        date_entry = ttk.Entry(date_frame, width=15, font=("微软雅黑", 12))
        ...
        time_entry = ttk.Entry(date_frame, width=8, font=("微软雅黑", 12))
        ...
        start_date_entry = ttk.Entry(range_frame, width=15, font=("微软雅黑", 12))
        ...
        end_date_entry = ttk.Entry(range_frame, width=15, font=("微软雅黑", 12))
        ...
        def calculate_date():
            try:
                ...
                # 添加结果到表格
                tree.insert("", tk.END, values=(test_date.strftime("%Y-%m-%d"), weekday, lunar_calc, lunar_auth, sizhu_calc, sizhu_auth))
            except ValueError as e:
        ...
        def test_date_range():
            try:
                ...
                while current_date <= end_date:
                    ...
                    # 计算农历
                    try:
                        lunar = get_lunar_date(current_date)
                        lunar_calc = f"{lunar.get('month', '未知')}{lunar.get('day', '未知')}"
                    except Exception as e:
                        lunar_calc = f"错误：{str(e)}"
                    ...
                    if HAS_SXTWL:
                        try:
                            import sxtwl
                            day = sxtwl.fromSolar(current_date.year, current_date.month, current_date.day)
                    ...
                    # 添加结果到表格
                    tree.insert("", tk.END, values=(current_date.strftime("%Y-%m-%d"), weekday, lunar_calc, lunar_auth, sizhu_calc, sizhu_auth))
        ...
        ttk.Button(button_frame, text="计算当前日期", command=calculate_date, width=15).pack(side=tk.LEFT, padx=8)
        ttk.Button(button_frame, text="测试选定日期", command=calculate_date, width=15).pack(side=tk.LEFT, padx=8)
        ttk.Button(button_frame, text="测试日期范围", command=test_date_range, width=15).pack(side=tk.LEFT, padx=8)
        ttk.Button(button_frame, text="清空结果", command=clear_results, width=15).pack(side=tk.LEFT, padx=8)
        ttk.Button(button_frame, text="关闭", command=window.destroy, width=15).pack(side=tk.RIGHT, padx=8)
        
        # 结果显示区域
        result_frame = ttk.LabelFrame(main_frame, text="测试结果", padding="20")
        ...
        # 创建表格
        tree = ttk.Treeview(table_frame, columns=("date", "weekday", "lunar_calc", "lunar_auth", "sizhu_calc", "sizhu_auth"), show="headings")
