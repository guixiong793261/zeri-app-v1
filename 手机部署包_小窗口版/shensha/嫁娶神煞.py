...
class MarriageShenShaChecker(ShenShaChecker):
    ...
    def check(self, sizhu, owners=None):
        ...
        # 先提取新娘/新郎信息
        print(f"[嫁娶神煞] 开始检查，owners数量: {len(owners) if owners else 0}", flush=True)
        if owners:
            for owner in owners:
                ...
                print(f"[嫁娶神煞] 事主: {name}, 性别: {gender}, 年干: {year_gan}, 生肖: {zodiac}", flush=True)
                
                if gender == '女':
                    self.bride_gan = year_gan
                    self.bride_zhi = zodiac or year_zhi
                    print(f"[嫁娶神煞] 设置新娘年干: {self.bride_gan}, 生肖: {self.bride_zhi}", flush=True)
                elif gender == '男':
                    self.groom_gan = year_gan
                    self.groom_zhi = zodiac or year_zhi
        
        print(f"[嫁娶神煞] 新娘年干: {self.bride_gan}, 月支: {sizhu.get('month_zhi', '')}", flush=True)
        ...
        print(f"[嫁娶神煞] 检查完成，神煞列表: {[s.get('name', '') for s in self.shensha_list]}", flush=True)
