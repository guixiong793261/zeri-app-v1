...
def verify_leap_year(year, leap_month):
    """验证有闰月年份的月柱计算"""
    print('=' * 60)
    print('%d年（闰%d月）月柱验证' % (year, leap_month))
    print('=' * 60)
    print()
    ...
    print('节气时间:')
    for i in range(24):
        dt = jd_to_datetime(jq_list[i].jd)
        print('  %s: %s' % (jq_names[i], dt))
    print()
...
def main():
    ...
    for year in range(1970, 2030):
        ...
        if leap_month > 0:
            leap_years.append((year, leap_month))
    
    print('有闰月的年份（1970-2030）:')
    for year, leap_month in leap_years:
    ...
    for year, leap_month in test_years:
        tests, errors = verify_leap_year(year, leap_month)
        total_tests += tests
        total_errors += errors
