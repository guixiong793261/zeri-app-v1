...
def _calculate_for_fire(wuxing_counts):
    ...
    # 关键修正：考虑木生火的影响
    # 当木很旺时，即使火的直接分数不高，也视为火旺
    adjusted_fire_count = fire_count
    if wood_count > 3.0:
        # 木旺生火，木越旺，火的实际力量越强
        # 木生火的加成：木分数 * 0.5（因为木是火的源头）
        adjusted_fire_count = fire_count + wood_count * 0.5
    
    if adjusted_fire_count > 5.0:
        # 火太旺，喜水克火，金耗火
        return "水、金", "水"
    elif fire_count > 4.0:
        # 火旺，喜水克火，金耗火
        ...
        # 综合判断：如果木偏旺但火不弱，也应取水金
        # 木>3.0且火>1.5时，视为木火通明但需要水来调和
        if wood_count > 3.0 and fire_count > 1.5:
            # 木旺生火，火已不弱，需要水来调和
            return "水、金", "水"
        # 一般情况，喜木生火
        return "木、火", "火"
