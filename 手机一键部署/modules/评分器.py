...
class Scorer:
    ...
    def score(self, sizhu, event_type, owners=None, house_type=None, shan_xiang=None,
        ...
        try:
            ...
            if event_type == '嫁娶':
                ...
                try:
                    ...
                    if date_obj and not sizhu.get('is_sizhu_input', False):
                        ...
                        # 综合得分 = 婚嫁评分 + 喜用神得分 + 月令得分 + 黄道得分 + 太阳太阴得分
                        total_score = marriage_result['score'] + xishen_score + yueling_score + huangdao_score + sun_moon_score
            ...
            # 计算最终得分：五行评分 + 月令得分 + 喜用神得分 + 黄道得分 + 太阳太阴得分
        ...
        if event_type in ['修造', '动土', '装修', '入宅']:
            ...
            if sansha_fang:
                ...
                if shan_xiang:
                    ...
                    # 检查山向是否在煞方
                    if shan_xiang_dir in sansha_fang:
                        analysis['reasons'].append(f"所选山向{shan_xiang}犯月三煞，不利修造")
            ...
            # 增强山向分析：考虑所有山向的相关方位
            if shan_xiang and event_type in ['修造', '动土', '装修', '入宅']:
                # 定义各山向的相关方位（二十四山）
                mountain_related = {
                    '乾': ['乾', '戌', '亥'],
                    '坤': ['坤', '未', '申'],
                ...
                # 检查该山向是否在定义的相关方位中
                if shan_xiang_dir in mountain_related:
                    related_dirs = mountain_related[shan_xiang_dir]
                    
                    # 检查月三煞是否影响山向相关方位
                    if sansha_fang:
                        for fang in related_dirs:
                            if fang in sansha_fang:
                                analysis['reasons'].append(f"月三煞在{fang}方，冲克{shan_xiang_dir}山")
                                analysis['details'][f'{shan_xiang_dir}山犯月三煞'] = True
                                analysis['score'] -= 50
                                analysis['level'] = '凶'
                                analysis['suitable'] = False
                    ...
                    if yuepo_fang:
                        if yuepo_fang in related_dirs:
                            analysis['reasons'].append(f"月破在{yuepo_fang}方，冲克{shan_xiang_dir}山")
                            analysis['details'][f'{shan_xiang_dir}山犯月破'] = True
                            analysis['score'] -= 50
                            analysis['level'] = '凶'
                            analysis['suitable'] = False
                ...
                if '山' in shan_xiang:
                    ...
                    if len(parts) > 1:
                        ...
                        # 检查朝向
                        if xiang and xiang in mountain_related:
    ...
    def _check_wu_xing(self, sizhu, event_type, owners, house_type, shan_xiang,
        ...
        # 新增：忌事一票否决机制
        # 如果忌事中包含当前事项类型（如嫁娶日课忌嫁娶），直接判定为不合格
        for ji in self.ji_list:
            if event_type in ji or (isinstance(ji, dict) and event_type in ji.get('name', '')):
                # 生成完整的五行分析信息
                details = self._generate_wu_xing_details(sizhu, owners)
                details['ji_list'] = self.ji_list
                return {
